...
```ts
// The workhorse: critically-damped-ish, no visible oscillation
const SNAP = { damping: 30, stiffness: 400, mass: 1 };
// Playful: one soft overshoot — use sparingly (celebrations, mascots)
const POP = { damping: 15, stiffness: 300, mass: 1 };

offset.value = withSpring(dest, { ...SNAP, velocity: event.velocityY });
```
...
const pan = Gesture.Pan()
  .onChange((e) => { offset.value += e.changeY; })          // worklet
  .onEnd((e) => {
    const dismiss = offset.value > H * 0.3 || e.velocityY > 800;
    offset.value = withSpring(dismiss ? H : 0, { ...SNAP, velocity: e.velocityY });
    if (dismiss) runOnJS(onClose)();
  });
```

Rules:
- Never read/write React state inside `onChange`. `runOnJS` only at gesture
  end, for navigation/effects.
